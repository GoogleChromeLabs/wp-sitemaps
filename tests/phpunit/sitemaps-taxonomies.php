...
class Test_Core_Sitemaps_Taxonomies extends WP_UnitTestCase {
	...
	public function test_get_sitemap_entries_custom_taxonomies() {
		...
		$this->assertContains( 'http://' . WP_TESTS_DOMAIN . '/?sitemap=taxonomy&sitemap-sub-type=public_taxonomy&paged=1', $entries, 'Public Taxonomies are not in the index.' );
		$this->assertNotContains( 'http://' . WP_TESTS_DOMAIN . '/?sitemap=taxonomy&sitemap-sub-type=private_taxonomy&paged=1', $entries, 'Private Taxonomies are visible in the index.' );
	}
}
